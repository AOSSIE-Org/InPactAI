...
export const CreatorMatchCard: React.FC<CreatorMatchCardProps> = ({
  ...
  return (
    <>
      <div className="bg-white rounded-2xl shadow-md p-6 px-4 flex flex-col items-center w-full max-w-xs border border-gray-100 relative min-h-[440px]">
        ...
        <div className="flex flex-col items-center gap-2 mt-8 mb-2">
          <Avatar className="h-16 w-16">
            ...
            <AvatarFallback className="bg-gray-200">{name.slice(0,2).toUpperCase()}</AvatarFallback>
          </Avatar>
        ...
        <div className="flex items-center gap-2 w-full justify-center mb-2">
          ...
          <div className="flex-1 h-2 rounded bg-gray-200 mx-2 min-w-[60px] max-w-[80px]">
        ...
        {requestSent && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-green-100 text-green-700 px-4 py-2 rounded-full text-sm shadow">Request Sent!</div>
        )}
      </div>
      <ViewProfileModal
        open={showProfile}
